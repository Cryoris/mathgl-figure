...
class Figure {
  ...
  private:
    ...
    std::vector<std::unique_ptr<mglData>[3]> data_; // plot data
...
void Figure::plot(const xVector& x, const yVector& y, const std::string style, const char* legend)
{
  ...
  std::unique_ptr<mglData> xd_ptr(new mglData(xd));
  std::unique_ptr<mglData> yd_ptr(new mglData(yd));

  data_.push_back({std::move(xd_ptr), std::move(yd_ptr), nullptr});
...
void Figure::plot3(const xVector& x, const yVector& y, const zVector& z, const std::string style, const char* legend)
{
  ...
  std::unique_ptr<mglData> xd_ptr(new mglData(xd));
  std::unique_ptr<mglData> yd_ptr(new mglData(yd));
  std::unique_ptr<mglData> zd_ptr(new mglData(zd));

  data_.push_back({std::move(xd_ptr), std::move(yd_ptr), std::move(zd_ptr)});
